...
public abstract class TemporalParserTest<T> {
    ...
    protected static final String DATE_TIME = "1985/10/24-21:20:42";
    protected static final String DATE = "2012/06/29";

    private TemporalParser<T> parser;
    private T expectedObject;
    private boolean timestamp;

    protected TemporalParserTest(TemporalParser<T> parser, T expectedObject, boolean timestamp) {
        this.parser = parser;
    ...
    public void testConvertIncorrectTimeStringToTime() {
        if (timestamp) {
            try {
                parser.convertFromString(timestamp ? DATE : DATE_TIME);
